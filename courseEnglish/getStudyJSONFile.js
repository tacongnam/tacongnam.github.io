    ...
    for (var i = 0; i < numpart - 1; i++) {
        ...
        for (var j = 0; j < sub_part_len; j++) {
            ...
            if(getElement.question_content != "") {
                HTMLstring += '<div class="question"><div class="question-title">' + getElement.question_content + '<br>' + getElement.answer_content + '</div>';
            
                HTMLstring += '<div class="question-note">' + getElement.answer_note + '</div></div>';
                
                HTMLstring += '<div class="check-status"><div class="part-instruction">' + getElement.part_instruction + '</div><button class="checkBtn" onclick="checkPart(' + (startPart.length + 1) + ')">' + getElement.button_show + '</button><div class="course-part"></div></div>';
